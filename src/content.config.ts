...
const work = defineCollection({
  ...
  schema: ({ image }) =>
    z.object({
      ...
      tag: z.string(),
      /**
       * An escape hatch for a screenshot that does not follow the naming
       * convention, as a path relative to this file. Covers are normally
       * matched to their project by id, so this is usually absent; see
       * src/lib/covers.ts.
       *
       * `image()` rather than a string: it resolves the file through
       * `astro:assets`, which means the same build-time processing the
       * convention-matched covers get, and a build that fails on a path
       * pointing at nothing rather than a page rendering a broken image.
       */
