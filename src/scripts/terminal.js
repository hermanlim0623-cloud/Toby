...
//
// The status line decodes out of noise rather than being swapped, which
// is the one moment in this section that should read as a machine
// acquiring a signal instead of text changing.
import { decodeInto } from './textFx.js';
...
export function createTransmission(gsap, ScrollTrigger, prefersReducedMotion) {
  ...
  tl.call(() => { decodeInto(status, 'ESTABLISHING CONNECTION', { duration: 700 }); })
    .to(obj, {
      p: 100,
      duration: 1.8,
      ease: 'steps(24)',
    ...
    .call(() => { decodeInto(status, 'CHANNEL READY', { duration: 600 }); })
